...
public class HomeActivity extends BaseActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    // SparseArray<String> 类似于HashMap<Integer,String> 使用时只需要制定value类型。
    // 1.具有比HashMap在正序插入上更快,不过在倒序插入或查找上速度差了很多(SparseArray每次在插入都要使用二分查找判断是否有相同的值被插入).
    // 2.SparseArray在内存使用上面永远优于HashMap。
